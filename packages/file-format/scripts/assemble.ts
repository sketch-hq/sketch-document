 ...
 * their file name.
 */
const loadSchema = (path: string): Promise<JSONSchema7> =>
  new Promise((resolve, reject) => {
    try {
      const schema = {
        ...(yaml.load(fs.readFileSync(path, 'utf8')) as object),
        $id: `#${pathToId(path)}`,
      }
      resolve(schema)
    } catch (e) {
 ...
 * in-place.
 */
const prune = (schema: any) => {
  let ids: string[] = []
  const scan = (o: any) => {
    if (typeof o !== 'object') return
    for (let [k, v] of Object.entries(o)) {
      if (k === '$ref') {
        const id = [...(v as string).split('/')].pop()
        if (!ids.includes(id!)) {
          ids.push(id!)
 ...
 */
const setAdditionalPropsFalse = (schema: any) => {
  const scan = (o: any) => {
    if (typeof o !== 'object') return
 ...
 * `optional` array.
 */
const setAllPropsRequired = (schema: any) => {
  const scan = (o: any) => {
    if (typeof o !== 'object') return
 ...
 */
const assemble = async (entry: string) => {
